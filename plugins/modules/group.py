...
class pfSenseGroup(object):
    ...
    def add(self, group):
        ...
        if group_elt is None:
            ...
            self.pfsense.write_config(descr='ansible pfsensible.core.group added %s' % (group['name']))
        else:
            changed = self.pfsense.copy_dict_to_element(group, group_elt)
            if self.module.check_mode:
                self.module.exit_json(changed=changed)
            if changed:
                self.pfsense.write_config(descr='ansible pfsensible.core.group updated "%s"' % (group['name']))
        self.module.exit_json(changed=changed)
    ...
    def remove(self, group):
        ...
        if group_elt is not None:
            ...
            changed = True
            self.pfsense.write_config(descr='ansible pfsensible.core.group removed "%s"' % (group['name']))
        self.module.exit_json(changed=changed)
